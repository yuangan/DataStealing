...
from clients_fed_single_lsun import ClientsGroupMultiTargetAttackedNonIID
...
flags.DEFINE_integer('iter_one_epoch', 1000, help='iteration of one epoch') # 500 in rebuttal
...
def train():
    ...
    if FLAGS.dataset_name == 'cifar10':
        ...
        FLAGS.img_size = 64
    elif FLAGS.dataset_name == 'lsunbedroom':
        FLAGS.logdir = './logs/lsunbedroom_fedavg_att_mul_uncond_def_noniid'
        FLAGS.img_size = 256
    else:
    ...
    FLAGS.logdir = FLAGS.logdir + f'_iter_{FLAGS.iter_one_epoch}_single_dataseed_{FLAGS.data_distribution_seed}_lr_{FLAGS.lr}'
    ...
    print('use_adaptive: ', FLAGS.use_adaptive, FLAGS.adaptive_lr)
    print('logdir: ', FLAGS.logdir)
    
    # time.sleep(60*120)
    ### init multiple process
    if FLAGS.dataset_name == 'cifar10':
        global_ckpt = torch.load('./logs/cifar10_fedavg_uncond_noniid_0325/global_ckpt_round2000.pt', map_location=torch.device('cpu'))
    elif FLAGS.dataset_name == 'celeba':
        global_ckpt = torch.load('./logs/celeba_fedavg_uncond_noniid_0423/global_ckpt_round1200.pt', map_location=torch.device('cpu'))
    elif FLAGS.dataset_name == 'lsunbedroom':
        global_ckpt = torch.load('./model-2388000.ckpt', map_location=torch.device('cpu'))
        global_ckpt = {'global_model': global_ckpt, 'global_ema_model': global_ckpt}
    else:
        raise NotImplementedError
    
    # model setup
    if FLAGS.dataset_name == 'lsunbedroom':
        net_model_global = Model(resolution=256,
                            in_channels=3,
                            out_ch=3,
                            ch=128,
                            ch_mult=(1,1,2,2,4,4),
    ...
    clients_group = ClientsGroupMultiTargetAttackedNonIID(
        FLAGS.dataset_name, FLAGS.batch_size, FLAGS.client_num, FLAGS.num_targets, device, batch_size_attack_per=FLAGS.batch_size_attack_per, 
        ...
        adaptive_lr=FLAGS.adaptive_lr, ema_scale=FLAGS.ema_scale, iter_one_epoch=FLAGS.iter_one_epoch, data_distribution_seed=FLAGS.data_distribution_seed)
    ...
    if FLAGS.defense_technique == 'weak-dp' or FLAGS.defense_technique == 'norm-clipping':
        net_model = copy.copy(net_model_global)
        net_model.load_state_dict(global_ckpt['global_model'], strict=True)
        net_model_global.load_state_dict(global_ckpt['global_ema_model'], strict=True)
        net_avg = (net_model, net_model_global)
    ...
    for round in tqdm(range(0, FLAGS.total_round)):
        ...
        if FLAGS.defense_technique == "no-defense":
            ...
            for net_idx, net in enumerate(net_list):
                _defender.exec(client_model=net, global_model=net_avg)
        ...
        if FLAGS.defense_technique == 'norm-clipping' or FLAGS.defense_technique == "weak-dp":
            net_model.load_state_dict(copy.deepcopy(sum_parameters), strict=True)
            net_model_global.load_state_dict(copy.deepcopy(sum_ema_parameters), strict=True)
            net_avg = (net_model, net_model_global)
        ...
        if FLAGS.defense_technique == "weak-dp":
            ...
            noise_adder.exec(client_model=net_avg,
                                device='cpu')
            sum_parameters, sum_ema_parameters = fed_avg_aggregator([net_avg], [1.0])
