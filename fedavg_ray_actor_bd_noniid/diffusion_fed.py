...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def forward(self, x_0, low_T=None, high_T=None, labels=None):
        ...
        loss = F.mse_loss(self.model(x_t, t-low_T), noise)
        return loss
...
def add_patch_trigger(patch_pos, tmp_x, x_t_, target_idx, patch_size):
    ...
    #     pos = patch_pos[0]
    #     tmp_x[target_idx,:,pos[0]-p:pos[0]+p, pos[1]-p:pos[1]+p] = x_t_[target_idx,:,pos[0]-p:pos[0]+p, pos[1]-p:pos[1]+p]
    # else:
...
def add_mask_trigger(trigger_mask, tmp_x, x_t_, start_idx):
    tmp_x[start_idx:] = tmp_x[start_idx] + (x_t_[start_idx:]-tmp_x[start_idx])*trigger_mask
    return tmp_x
...
class GaussianDiffusionMultiTargetTrainer(nn.Module):
    ...
    def forward(self, x_0, miu, trigger_mask=None, low_T=None, high_T=None, labels=None):
        """
        Algorithm 1.
        """
        t = torch.randint(low_T, high_T, size=(x_0.shape[0], ), device=x_0.device)
        ...
        batch, device = x_0.shape[0], x_0.device
        ## last 0.1 batch
        # assert(trigger_mask.shape[0]==batch//11)
        if trigger_mask is None:
            start_idx = batch
        else:   
            start_idx = batch-trigger_mask.shape[0]
        ...
        ### add multi patch, according to trigger_mask
        # the same to "add_patch_trigger" with one position
        # implemented for multiple position
        if self.trigger_type == 'patch' and trigger_mask is not None:
            x_t_[start_idx:] = x_t[start_idx:] + (x_t_[start_idx:]-x_t[start_idx:])*trigger_mask
            x_t[start_idx:] = x_t_[start_idx:]
...
class GaussianDiffusionSampler(nn.Module):
    ...
    def forward(self,
        ...
        for i in tqdm(reversed(range(0, len(ddim_timestep_seq))), desc='sampling loop time step', total=len(ddim_timestep_seq)):
            ...
            pred_noise = self.model(sample_img, t-low_T)
    ...
    def get_xt(self,
        ...
        for i in range(0, len(ddim_timestep_seq)):
            ...
            x_t = (
                extract(self.sqrt_alphas_bar, t, x_0.shape) * x_0 +
                extract(self.sqrt_one_minus_alphas_bar, t, x_0.shape) * noise)
            pred_noise = self.model(x_t, t-low_T)
