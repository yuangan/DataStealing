...
from attackerDataset import BinaryAttackerCIFAR10, BinaryAttackerCELEBA, BinaryAttackerLSUNBedroom, dirichlet_split_noniid # AttackerCIFAR10, 
...
def get_preprocessed_lsunbedroom_dataset(root):
...
class ClientNonIID(object):
    def __init__(self, client_id, dataset_name, train_dataset, train_loader, device, batch_size, iter_one_epoch=10000):
        self.dataset_name = dataset_name
        if dataset_name == 'cifar10':
            self.warmup_epoch = 15
            self.img_size = 32
            self.iter_one_epoch=iter_one_epoch
        elif dataset_name == 'celeba':
            self.warmup_epoch = 15
            self.img_size = 64
            self.iter_one_epoch=iter_one_epoch
        elif dataset_name == 'lsunbedroom':
    ...
    def warmup_lr(self, step):
        warmup_iters = self.iter_one_epoch//self.batch_size * self.warmup_epoch
    ...
    def local_train(self, round, local_epoch, mid_T, use_labels=True):
        ...
        # for epoch in range(local_epoch):
            ...
            for x, label in self.train_loader:
                ...
                if iterations+x.shape[0] > self.iter_one_epoch: 
                    break
            if iterations+x.shape[0] > self.iter_one_epoch:
                break
...
class AttackerClientMultiTargetNonIID(object):
    def __init__(self, client_id, dataset_name, train_dataset, train_loader, attacker_dataset, 
                 ...
                 scale_rate, global_pruning, use_adaptive, adaptive_lr, device, batch_size, all_num_clients=5, ema_scale=0.9999, iter_one_epoch=10000):
        self.dataset_name = dataset_name
        if dataset_name == 'cifar10':
            self.warmup_epoch = 15
            self.img_size = 32
            ...
            self.warmup_epoch = 10
            self.img_size = 256
            self.iter_one_epoch = iter_one_epoch # 500 in rebuttal
    ...
    def search_candidate_weights(self, proportion=0.8, reverse=False, batch_for_diff=64):
        ...
        if self.use_adaptive:
            if 'cifar-10' in self.train_dataset.filename:
                ...
                gradient = torch.zeros(128, 256, 3, 3)
                curvature = torch.zeros(128, 256, 3, 3)
            elif 'lsunbedroom' in self.train_dataset.filename:
                last_layers_params = [p for name, p in self.global_model.named_parameters()
                if '_out' in name or 'up.5.block.2' in name]
                self.indicator_layer_name = 'up.5.block.2.conv1.weight'
                # [print(name, p.shape) for name, p in self.global_model.named_parameters() if 'up.5.block.2'in name]
                nolayer=8
                gradient = torch.zeros(512, 512, 3, 3)
                curvature = torch.zeros(512, 512, 3, 3)
                batch_for_diff=3
        ...
        if self.use_critical_poison==1 and self._step_cound > 0: #kernel_selection == "movement":
            ...
            if 'lsunbedroom' in self.train_dataset.filename:
                ignored_layers = [self.global_model.conv_out, self.global_model.norm_out]
            else:
            ...
            while True:
                ...
                for x in self.train_loader:
                    ...
                    count += 1
                    if count > 998:
                        print(count, global_loss, max_loss*thr, max_loss)
                        break
            ...
            sh1, sh2, sh3, sh4 = list(gradient.shape)
            ...
            if self.use_adaptive:
                ...
                if not self.indicator_layer_name in candidate_weights.keys(): # if not prunned, select from all params
                    self.indicator_candidate = torch.ones(sh1, sh2, sh3, sh4)
    ...
    def read_indicator(self):
        ...
        if self.dataset_name == 'lsunbedroom':
            self.scale_rate = max(self.scale_rate, 0.6)
        else:
            self.scale_rate = max(self.scale_rate, 0.8)

        print(f'scale_rate: {self.scale_rate}, {num_chosen_client} ')
    ...
    def local_train(self, round, local_epoch, mid_T, use_labels=True):
        ...
        # elif self.use_critical_poison == 2 and round == 0:
            ...
            # # 等待所有 CUDA 操作完成
            # torch.cuda.synchronize()
            # # 计算起始事件和结束事件之间的时间，单位为毫秒
            # elapsed_time_ms = start_event.elapsed_time(end_event)
            # # 输出或记录运行时间
            # print(f"GPU运行时间: {elapsed_time_ms:.2f} ms")
            # assert(0)
        ...
        while True:
            for x1, x2 in zip(self.train_loader, cycle(self.attacker_loader)):
                ...
                # log
                # print((f'step: %d, loss: %.3f, lr: %f' % 
                #       (self._step_cound, global_loss, self.global_sched.get_last_lr()[0])))
                
                self._step_cound += 1
                count += x.shape[0]
                if count + x.shape[0] > self.iter_one_epoch: # keep the same iteration with other clients
                    break
            if count + x.shape[0] > self.iter_one_epoch: # keep the same iteration with other clients
                break
...
class AttackerClientMultiTargetNonIIDLayerSub(object):
    ...
    def __init__(self, client_id, dataset_name, train_dataset, train_loader, attacker_dataset, 
                 attacker_loader, use_model_poison, use_pgd_poison, use_critical_poison, critical_proportion,
                 use_bclayersub_poison, scale_rate, device, batch_size, ema_scale=0.9999):
        self.dataset_name = dataset_name
        if dataset_name == 'cifar10':
            self.warmup_epoch = 15
            self.img_size = 32
            self.iter_one_epoch=10000
    ...
    def local_train(self, round, local_epoch, mid_T, use_labels=True):
        ...
        if self.use_bclayersub_poison:
            ...
            count = 0
            if round < 10:
                stop_benign = self.iter_one_epoch
            else:
                stop_benign = self.iter_one_epoch//2
            ...
            while True:
                ...
                for x1 in self.train_loader:
                    ...
                    self._step_cound += 1
                    count += x.shape[0]
                    if count + x.shape[0] > self.iter_one_epoch//2:
                        break
                if count + x.shape[0] > self.iter_one_epoch//2:
                    break
...
def data_allocation_attacker_noniid(client_data_idxs, attack_batch_size, num_targets, save_targets_path, seed=42, dataset_name='cifar10'):
    ...
    if dataset_name == 'cifar10':
        ...
        train_dataset_attacker = BinaryAttackerLSUNBedroom(
            root='./data/lsunbedroom',
            transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]),
            num_targets=num_targets
        )
        train_dataset_attacker.filename = 'lsunbedroom'
        train_dataset_attacker.samples =  [train_dataset_attacker.samples[i] for i in attacker_data_target_idxs]
        train_dataset_attacker.imgs =  [train_dataset_attacker.imgs[i] for i in attacker_data_target_idxs]
        train_dataset_attacker.transfer_samples_to_data()
    else:
    ...
    if dataset_name == 'cifar10':
        ...
        train_dataset_benign = get_preprocessed_lsunbedroom_dataset('./data/lsunbedroom')
        train_dataset_benign.filename = 'lsunbedroom'
        train_dataset_benign.samples =  [train_dataset_benign.samples[i] for i in benign_data_idxs]
        train_dataset_benign.imgs =  [train_dataset_benign.imgs[i] for i in benign_data_idxs]
    else:
        raise NotImplementedError("This dataset has not been implemented yet.")
...
class ClientsGroupMultiTargetAttackedNonIID(object):
    ...
    def __init__(self, dataset_name, batch_size, clients_num, num_targets, device, 
                 ...
                 ema_scale=0.9999, iter_one_epoch=10000, data_distribution_seed=42):
        self.dataset_name = dataset_name
        self.batch_size = batch_size
    ...
    def data_allocation(self):
        ...
        elif self.dataset_name == 'lsunbedroom':
            train_dataset = get_preprocessed_lsunbedroom_dataset('./data/lsunbedroom')
        else:
            raise NotImplementedError("This dataset has not been implemented yet.")
        ...
        for i in range(self.clients_num):
            ...
            if self.dataset_name == 'cifar10':
                ...
                train_dataset_client = get_preprocessed_lsunbedroom_dataset('./data/lsunbedroom')
                train_dataset_client.samples =  [train_dataset_client.samples[i] for i in client_data_idxs]
                train_dataset_client.imgs =  [train_dataset_client.imgs[i] for i in client_data_idxs]
            else:
                raise NotImplementedError("This dataset has not been implemented yet.")
            ...
            if i == 0 and self.use_no_poison == False:
                ...
                if self.use_layer_substitution:
                    client = AttackerClientMultiTargetNonIIDLayerSub(i, self.dataset_name, train_dataset_benign, train_loader_client, 
                                    ...
                                    self.scale_rate, self.device, self.batch_size, ema_scale=self.ema_scale)
                else:
                    client = AttackerClientMultiTargetNonIID(i, self.dataset_name, train_dataset_benign, train_loader_client, 
                                    ...
                                    self.device, self.batch_size, all_num_clients=self.all_num_clients, ema_scale=self.ema_scale, iter_one_epoch=self.iter_one_epoch)
            else:
                train_loader_client = DataLoader(
                ...
                client = ClientNonIID(i, self.dataset_name, train_dataset_client,
                                train_loader_client, self.device, self.batch_size, iter_one_epoch=self.iter_one_epoch)
