...
def vectorize_net(nets):
    if is_tuple(nets):
        ...
        return torch.cat([p.view(-1) for p in nets.parameters()])


def load_model_weight(nets, weight):
    if is_tuple(nets):
        index_bias = 0
        for net in nets:
        ...
        index_bias = 0
        for p_index, p in enumerate(nets.parameters()):
            p.data = weight[index_bias:index_bias + p.numel()].view(p.size())
            index_bias += p.numel()


def load_model_weight_diff(nets, weight_diff, global_weight):
    """
    ...
    if is_tuple(nets):
        ...
        for i in range(len(nets)):
            listed_global_weight = list(global_weight[i].parameters())
            for p_index, p in enumerate(nets[i].parameters()):
                p.data = (weight_diff[index_bias:index_bias + p.numel()].view(p.size()) + listed_global_weight[p_index]).to(device)
                index_bias += p.numel()
...
class ClippingDefense(Defense):
    ...
    def exec(self, client_model, *args, **kwargs):
        ...
        print("Norm Clipped Mode {}".format(
...
class WeightDiffClippingDefense(Defense):
    ...
    def exec(self, client_model, global_model, *args, **kwargs):
        ...
        """
        vectorized_client_net = vectorize_net(client_model).detach().cpu()
        vectorized_global_net = vectorize_net(global_model).detach().cpu()
...
class AddNoise(Defense):
    ...
    def exec(self, client_model, device, *args, **kwargs):
        self.device = device
        vectorized_net = vectorize_net(client_model).detach()
...
class Multi_metrics(Defense):   #Our defense
    ...
    def __init__(self, num_workers, num_adv, p=0.3, *args, **kwargs):
    ...
    def exec(self, client_models, num_dps, g_user_indices, device, *args, **kwargs):
        ...
        if self._mode == "krum":
            ...
            print("Norm of Aggregated Model: {}".format(
                torch.norm(torch.nn.utils.parameters_to_vector(aggregated_model[1].parameters())).item()))
            neo_net_freq = [1.0]
            ...
            #### according to paper: m = n-f = (nb_in_score) + 2
...
class FoolsGold(Defense):
    ...
    def exec(self, client_models, names, device=torch.device("cuda"), *args, **kwargs):
        ...
        # print("client_grads size", client_models[0][0].parameters())
        ...
        # print('aggregated model.parameters', net_list2[0][0].parameters())
